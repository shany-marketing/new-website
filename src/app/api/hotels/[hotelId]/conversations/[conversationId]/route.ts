...
type Params = Promise<{ hotelId: string; conversationId: string }>;
...
export async function GET(
  ...
  const { hotelId, conversationId } = await params;
  ...
  const conv = await queryOne<{ id: string; title: string; created_at: string }>(
    ...
    [conversationId, hotelId, authResult.session.user.id]
  );
...
export async function PATCH(
  ...
  const updated = await queryOne(
    `UPDATE chat_conversations SET title = $1, updated_at = now()
     WHERE id = $2 AND hotel_id = $3 AND user_id = $4
     RETURNING id`,
    [title, conversationId, hotelId, authResult.session.user.id]
  );

...
const RUN_ID = "h9gXkd5g934YBMu46";
const DATASET_ID = "yRrJHJGyOKpGOSZ4U";
...
// Poll
console.log("Polling Booking run (residential proxy)...");
for (let i = 0; i < 180; i++) {
  const res = await fetch(`${BASE}/actor-runs/${RUN_ID}?token=${token}`);
  const data = await res.json();
  const status = data.data?.status;
  if (status === "SUCCEEDED") { console.log(`SUCCEEDED after ${i * 10}s`); break; }
...
for (const raw of all) {
  ...
  try {
    const res = await pool.query(
      ...
      [
        HOTEL_ID, "booking", String(raw.id), raw.checkInDate??null, raw.checkOutDate??null,
        raw.likedText?.trim()||null, raw.dislikedText?.trim()||null, raw.numberOfNights??null, raw.rating??null, raw.reviewDate??null,
        raw.reviewTitle?.trim()||null, raw.roomInfo?.trim()||null, raw.travelerType?.trim()||null, raw.userLocation?.trim()||null,
        raw.userName?hashPII(raw.userName):hashPII("anonymous"), cleanName(raw.userName),
        raw.reviewLanguage?.trim()||null, raw.helpfulVotes??null, raw.propertyResponse?.trim()||null,
